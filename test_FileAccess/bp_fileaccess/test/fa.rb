...
class TestFileAccess < Test::Unit::TestCase
  ...
  def setup
	...
	serviceLoc1 = File.join(ENV["HOME"], "Library", "Application Support",
                            "Yahoo!", "BrowserPlus", "Corelets")
	@r = File.join(serviceLoc1, "FileAccess", "1.0.11")
  ...
  def test_chunk
	...
	Dir.glob(File.join(File.dirname(__FILE__), "cases_chunk", "*.json")).each do |f|
		...
		@testid+=1
		#testid: 2 Chunksize => 0 should not cause invocation error, should cause Runtime error? <------------------------ BUG 213
		...
		@testid+=1
		#testid: 3, Negetive chunksize returns whole file. <------------------------ BUG 211
...
#Given a file and an optional offset and size, return a new file whose contents are a subset of the first.
  def test_slice
	...
	Dir.glob(File.join(File.dirname(__FILE__), "cases_slice", "*.json")).each do |f|
		...
		@binfile_path = File.join(File.dirname(File.expand_path(__FILE__)), "testFiles", "services.bin")
		@binfile_uri = (( @binfile_path[0] == "/") ? "file://" : "file:///" ) + @binfile_path
		...
		@testid+=1
		# testid: 3, slice a binary file --- should raise Runtime error? <------------------- BUG 213 -- bug fixed, or no longer present.
		...
		#begin
		#	got = @s.slice({ 'file' => @textfile_uri, 'offset' => 1024000 })
		#	assert_runTime(got, false, $log, x, @testid)
		#rescue RuntimeError
		#	assert_runTime(got, true, $log, x, @testid)
		#end
